...
while game_counter <= games_count:
    ...
    while scheme_counter <= 12:
        check_scheme = browser.find_element_by_xpath('//*[@id="detail_0_team_stats"]/div/div/div/main/div[3]/div[2]/div[2]/table/tbody/tr['+str(game_counter)+']/td['+str(scheme_counter)+']')
        if check_scheme.text == '0':
            scheme_counter += 1
            continue
        scheme = browser.find_element_by_xpath('//*[@id="detail_0_team_stats"]/div/div/div/main/div[3]/div[2]/div[2]/table/tbody/tr['+str(game_counter)+']/td['+str(scheme_counter)+']/span')
        ...
        pd.DataFrame(data_table).to_csv(r'D:\intelligence-systems\origin_data_def\df_[%s][%s]%s.csv' % (game_name_1, game_name_2, scheme_counter-6))
        browser.implicitly_wait(10)
        scheme.click()
        scheme_counter += 1
